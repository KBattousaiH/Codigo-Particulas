...
Imag=['Bat3';'NiM3';'SST3';'SSS3'];
varn=[' m ';' m ';' ºC';'PSU'];
rang=[0,60 ; 0.07,0.09 ; 12.3,12.7 ; 32.1,32.25];
%% T S h SL
for i=1:4
f1(i)=figure(i);
   % sgtitle('SST','FontSize',22)
    ...
    m_grid('linewi',2,'tickdir','out','fontsize',5); % Tamaño nº coordenadas
...
%     %c = colorbar(j,'Position',[0.91 0.15 0.03 0.73]); 
     c = colorbar(j);
     set(get(c,'title'),'string','m/s'); caxis(j,[-0.035 0.035]);
        cmocean('balance');
%     cmocean('dense')
    set(gcf,'position',[10,10,600,500])
    saveas(f,'SSU3','png');
...
%     %c = colorbar(j,'Position',[0.91 0.15 0.03 0.73]); 
     c = colorbar(j);
     set(get(c,'title'),'string','m/s'); caxis(j,[-0.07 0.07]);
        cmocean('balance');
%     cmocean('dense')
    set(gcf,'position',[10,10,600,500])
    saveas(f,'SSV3','png');